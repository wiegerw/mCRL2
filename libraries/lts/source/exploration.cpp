...
void
lps2lts_algorithm::set_prioritised_representatives(next_state_generator::transition::state_probability_list& states)
{
...
lps::state lps2lts_algorithm::get_prioritised_representative(const lps::state& state1)
{
  ...
  while (true)
  {
    ...
    if (number[state] == 0)
    {
      ...
      for (auto i = m_generator->begin(state, m_prioritized_subset, &enumeration_queue); i; i++)
      {
...
void lps2lts_algorithm::value_prioritize(std::vector<next_state_generator::transition>& transitions)
{
  ...
  for (std::vector<next_state_generator::transition>::const_iterator i = transitions.begin();
       i != transitions.end(); ++i)
  {
    ...
    {
      data::data_expression result = m_generator->rewriter()(
              data::equal_to(lowest_value, i->action.actions().front().arguments().front()));
...
void lps2lts_algorithm::generate_lts_breadth_bithashing(
        const next_state_generator::transition::state_probability_list& initial_states)
{
  std::size_t current_state = 0;
  ...
  while (!m_must_abort && (state_queue.remaining() > 0) && (current_state < m_options.max_states) &&
         (!m_options.trace || m_traces_saved < m_options.max_traces))
  {
    const lps::state state = state_queue.get_from_queue();
    get_transitions(state, transitions, enumeration_queue);
    ...
    if (!m_options.suppress_progress_messages && ((current_state % 1000) == 0))
    {
      mCRL2log(status) << "monitor: currently at level " << m_level << " with "
                       ...
                       << " explored and " << m_num_states << " state" << ((m_num_states == 1) ? "" : "s") << " seen."
                       << std::endl;
    }
...
void
lps2lts_algorithm::generate_lts_depth(const next_state_generator::transition::state_probability_list& initial_states)
{
  ...
  while (!m_must_abort && (!stack.empty()) && (!m_options.trace || m_traces_saved < m_options.max_traces))
  {
    ...
    for (auto& tr: transitions)
    {
      if (add_transition(state, tr) && (current_state + stack.size() < m_options.max_states) &&
          (stack.size() < m_options.todo_max))
      {
...
void
lps2lts_algorithm::generate_lts_random(const next_state_generator::transition::state_probability_list& initial_states)
{
  if (++initial_states.begin() !=
      initial_states.end())  // There is more than one element in the set of initial states.
  {
    mCRL2log(warning)
      << "The initial state is not selected at random, conform its distribution. One specific state is chosen.";
  }
  ...
  while (!m_must_abort && current_state < m_options.max_states &&
         (!m_options.trace || m_traces_saved < m_options.max_traces))
  {
    get_transitions(state, transitions, enumeration_queue);
    ...
    if (!m_options.suppress_progress_messages && ((current_state % 1000) == 0))
    {
      mCRL2log(verbose) << "monitor: currently explored "
                        << m_num_transitions << " transition" << ((m_num_transitions == 1) ? "" : "s")
                        << " and encountered " << m_num_states << " unique state" << ((m_num_states == 1) ? "" : "s")
                        << std::endl;
    }
  }
