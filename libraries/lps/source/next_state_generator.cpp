...
void next_state_generator::summand_subset::build_pruning_parameters(const stochastic_action_summand_vector& summands)
{
        ...
        rewriter_substitution& sigma)
{
  ...
  else
  {
    ...
    typedef enumerator_algorithm_with_iterator<data::rewriter, enumerator_list_element_with_substitution<>, is_not_zero> enumerator_type;
    ...
    std::deque<enumerator_list_element_with_substitution<> > enumerator_solution_deque(1,
                                                                                       enumerator_list_element_with_substitution<>(
                                                                                               dist.variables(),
                                                                                               dist.distribution()));
    for (auto probabilistic_solution = enumerator.begin(sigma, enumerator_solution_deque);
...
void next_state_generator::iterator::increment()
{
  ...
  else
  {
    ...
    m_transition.m_other_target_states = resulting_state_probability_list;
  }
  ...
  for (std::size_t i = 0; i < m_summand->action_label.size(); i++)
  {
    ...
    actions[i] = process::action(m_summand->action_label[i].label,
                                 data_expression_list(arguments.begin(), arguments.end()));
  }
